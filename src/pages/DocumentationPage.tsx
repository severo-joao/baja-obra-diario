...
const samplePayloads: Record<string, string> = {
  "relatorio.criado": JSON.stringify({
    ...
    data: {
      ...
      data_relatorio: "2025-03-21",
      atividades_dia: "Concretagem do segundo pavimento...",
  ...
  "relatorio.atualizado": JSON.stringify({
    ...
    data: {
      id: "abc-123",
      changes: ["atividades_dia", "observacoes"],
    },
  }, null, 2),
  "cliente.cadastrado": JSON.stringify({
    ...
    data: {
      ...
      nome_empreitada: "Residencial Aurora",
    },
  }, null, 2),
};

export default function DocumentationPage() {
  const { webhooks, webhookLogs, addWebhook, updateWebhook, deleteWebhook, addWebhookLog } = useAppStore();
          ...
          <Card className="shadow-sm">
            ...
            <CardContent>
              <div className="flex flex-col sm:flex-row gap-3">
                ...
                <Button onClick={handleAddWebhook} className="bg-baja-orange hover:bg-baja-orange/90 text-accent-foreground">
                ...
                {webhooks.map((wh) => (
                  <div key={wh.id} className="flex flex-col sm:flex-row sm:items-center gap-3 p-3 rounded-lg bg-secondary/50">
                    ...
                    <div className="flex items-center gap-2">
                      <Switch checked={wh.active} onCheckedChange={(v) => updateWebhook(wh.id, { active: v })} />
                      <Button variant="outline" size="sm" onClick={() => handleTest(wh)}>
                        <Send className="h-3.5 w-3.5 mr-1" /> Testar
                      </Button>
                      <Button variant="ghost" size="icon" onClick={() => { deleteWebhook(wh.id); toast.success("Webhook removido!"); }} className="text-destructive hover:text-destructive">
                ...
                <div className="space-y-2">
                  {webhookLogs.slice(0, 10).map((log) => (
                    <div key={log.id} className="flex items-center gap-3 text-sm p-2 rounded bg-muted/50">
                      ...
                      <span className="text-xs text-muted-foreground">{format(new Date(log.timestamp), "dd/MM HH:mm")}</span>
          ...
          <Card className="shadow-sm">
            <CardContent className="p-6 prose prose-sm max-w-none">
              ...
              <p className="text-sm text-muted-foreground mb-4">
                Os webhooks enviam requisições POST para a URL configurada quando eventos ocorrem no sistema.
              </p>
